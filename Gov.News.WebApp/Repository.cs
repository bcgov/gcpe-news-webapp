...
namespace Gov.News.Website
{
    public class Repository
    {
        ...
        public async Task<IList<Minister>> GetMinistersAsync(IEnumerable<string> ministryKeys)
        {
            ...
            {
                ministers.Add(await GetDataModelAsync(ministryKey, () => ApiClient.Ministries.GetMinisterWithHttpMessagesAsync(ministryKey, APIVersion, MustRevalidateHeader())));
            }
            return ministers;
        }
        ...
        public async Task<Post> GetPostAsync(string key)
        {
            if (key != null)
            {
                try
                {
                    return await GetDataModelAsync(key, () => ApiClient.Posts.GetOneWithHttpMessagesAsync(key, APIVersion), MAX_NUM_CACHED_POSTS);
                }
                catch (Exception) { }
            }
            return null;
