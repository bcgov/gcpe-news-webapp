...
function ss_handleKey(e){
	...
	var queryForm = e.srcElement.form;
...
function ss_clear() {
	// NOP - need the function defined for headerFooter
}
...
function renderSearchQueries(data, table){
	...
	if(queryReturn != null && queryReturn.results.length > 0){
...
//		proxyPath = '/searchAdaptationLayerWS/suggest/querySuggest';
		// http://localhost:8080/searchAdaptationLayerWS/suggest/querySuggest?q=ms&max=10&site=default_collection&client=default_frontend
...
$(document).on("click", "table.ss-gac-m > tbody > tr", function() {
	...
	if(!$(this).hasClass("ss-gac-e")){
		...
		$form.find("[class*='searchbox'],input#keyword").val(selectedText);
		$form.submit();
	}
});
