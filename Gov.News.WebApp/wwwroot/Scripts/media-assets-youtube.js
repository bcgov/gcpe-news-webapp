var mediaAssetProxyUrl = youtubeProxyUrl;
...
function resizeEmbeddedMediaAssets() {
    ...
    for (j = 0; j < assets.length; j++) {
        ...
        $(placeholder).find(".overlay-container .outer .inner").css("top", offset).css("left", offset_width).css("visibility", "visible");
...
function closeYoutubeInstructions(closeButton) {
...
function returnMediaAssetCookie() {
    ...
    return [{ "name": "soundcloud", "enabled": 0 }, { "name": "youtube", "enabled": 0 }, { "name": "wowza", "enabled": 0 }];
}
